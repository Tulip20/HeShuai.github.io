...
# --- 取行操作 ---
print(t1[1])      # 取第2行(索引从0开始), 等价于 t1[1,:]
print('*'*20)

print(t1[1:])     # 取第2行到最后一行(连续多行)
print('*'*20)

print(t1[1:3,:])  # 取第2~3行(左闭右开, 不包含索引3)
print('*'*20)

print(t1[[0,2,3]])    # 取第1、3、4行(不连续多行, 用列表指定行索引)
print('*'*20)

print(t1[[0,2,3],:])  # 同上, 加 ':' 表示取这些行的所有列
print('*'*20)

# --- 取列操作 ---
print(t1[:,1])        # 取第2列(':' 表示所有行, 1 表示第2列)
print('*'*20)

print(t1[:,1:])       # 取第2列到最后一列(连续多列)
print('*'*20)

print(t1[:,[0,2,3]])  # 取第1、3、4列(不连续多列)
print('*'*20)

# --- 取单个值 ---
print(t1[2,3])        # 取第3行第4列的值, 即15
print('*'*20)
